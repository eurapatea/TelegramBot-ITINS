...
def send_welcome(message):
    ...
    bot.reply_to(message, "Привет Какой вопрос о недвижимости вас интересует?", reply_markup=markup)
...
@bot.message_handler(func=lambda message: message.text in ['Квартира для продажи', 'Дом для продажи', 'Земельный участок для продажи', 'Апартаменты для продажи', 'Коммерческая недвижимость для продажи'])
def specific_sale(message):
    markup = types.ReplyKeyboardMarkup(row_width=2)
    itembtn1 = types.KeyboardButton('💵 Продажа')
    itembtn2 = types.KeyboardButton('Назад')
    markup.add(itembtn1, itembtn2)
    bot.reply_to(message, "Выберите что вас интересует?", reply_markup=markup)
